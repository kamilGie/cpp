...
int main(){
    ...
    } while (choice < 1 && choice > 3)//tutaj trzeba jakos zmienic whila bo byl zly zmieniać && na || oraz jak zmienilisy --choice to trzeba zmienic tez te wartosci o jeden no i dodac ';'
    int* p = data[choice];
    ...
    if (*m == number) cout << "Trafiles!" << endl;                                                                            
    else cout << "Pudlo! Sprobuj jeszcze raz!" << endl;

    //bedac super poprawnym trzeba jeszcze dodac tutaj delete tych data bo stowrzylismy je dynamicznie ale to nie jest tez niezbednie bez tego program zadziala i jak sie skonczy sam je usunie
}